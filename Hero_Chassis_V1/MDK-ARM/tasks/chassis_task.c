...
/**
    * @brief  用于底盘的运动控制
    * @note   S2为UP由键盘控制,S2为MID由遥控控制,S2为DOWN开启小陀螺
    * @author 钟午杰
    * @param  None
    * @retval None
    */
void chassis_task(void const * argument)
{
  ...
  float degree=0;
	while(1)
	{		
      if (rc_device_get_state(&rc, RC_S2_MID))				//若sw2为MID，底盘跟随云台，速度由遥控器ch1-2控制，角速度PID由follow_relative_angle决定
      {//相当于右摇杆的两个通道控制x,y
				//(1684-364)/2
      ...
      if (rc_device_get_state(&rc, RC_S2_UP))			//若S2在上方,则由键盘控制
      {
        ...
        wz =2000;	//暂定转速为2000 
			...
			if(rc_device_get_state(&rc, RC_S2_DOWN2MID)){//S2为DOWN2MID，取消小陀螺，仍然接收遥控器数据
				if(degree<0.1&&degree>-0.1)  wz=2000;			//当转到枪管与车身平行时，停止旋转
				else wz=0;
			}
