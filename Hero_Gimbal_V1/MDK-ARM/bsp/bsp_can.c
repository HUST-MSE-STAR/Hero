...
moto_info_t motor_info[6];//其实只有5个电机
moto_info_t last_motor_info[6];
_Bool id_range;
uint8_t        protocol_packet[REF_PROTOCOL_FRAME_MAX_SIZE];//裁判系统的数据
/**
    * @brief  用于can通信初始化
    * @note   None
    * @author 周森
...
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *hcan)
{
   ...
   && (rx_header.StdId <  FEEDBACK_ID_BASE_6020 + MOTOR_MAX_NUM))                  // judge the can id
  {
	...
	else if(hcan->Instance == CAN1)//和底盘的通信
  {
    HAL_CAN_GetRxMessage(hcan, CAN_RX_FIFO0, &rx_header, rx_data); //receive can data
		if(rx_header.StdId==110) strcpy((char *)REFEREE_buf,(const char *)rx_data); //底盘CAN的ID是110
