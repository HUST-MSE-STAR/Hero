...
void rc_callback_handler(rc_info_t *rc, uint8_t *buff)
{
  ...
  rc->wheel = (buff[16] | buff[17] << 8) - 1024;
	...
	get_dr16_state(rc);//判断开关的state

//	osSignalSet(communication_taskhandle, RC_UPDATA);	
}
...
void get_dr16_state(rc_info_t *rc_dev)		
{
  ...
  else if(rc_dev->sw2 == 2)
  {
	...
	osSignalSet(communication_to_chasis_taskhandle, RC_UPDATA);	
}
...
static void uart_rx_idle_callback(UART_HandleTypeDef* huart)
{
    ...
    if (huart == &PC_HUART)
	{
		...
		if ((PC_MAX_LEN - dma_current_data_counter(huart->hdmarx->Instance)) == PC_BUFLEN)
		{
			calc_rev_buff();
...
void dbus_uart_init(void)
{
	...
	__HAL_UART_ENABLE_IT(&DBUS_HUART, UART_IT_IDLE);//IDLE中断：当接收到1个字节，就会产生RXNE中断，当接收到一帧数据，就会产生IDLE中断。比如给单片机一次性发送了8个字节，就会产生8次RXNE中断，1次IDLE中断。）
	uart_receive_dma_no_it(&DBUS_HUART, dbus_buf, DBUS_MAX_LEN);//在这里指定好了接收缓冲区
	
	__HAL_UART_CLEAR_IDLEFLAG(&PC_HUART);
	__HAL_UART_ENABLE_IT(&PC_HUART, UART_IT_IDLE);
	uart_receive_dma_no_it(&PC_HUART, PC_buf, PC_MAX_LEN);
